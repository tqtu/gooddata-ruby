...
describe 'Should upgrade custom v2 for project', :vcr, :constraint => 'slow' do
  ...
  it 'upgrade all date dimension to custom v2' do
    ...
    date_old.each do |date|
      ...
      expect(GoodData::LCM2::SynchronizeDateDimension::DATE_DIMENSION_OLD.any? { |e| date[:urn]&.include?(e) }).to be_truthy
    end

    message = GoodData::LCM2::SynchronizeDateDimension::get_upgrade_message(true, nil)
    ...
    date_new.each do |date|
      expect(%w[datecustom dategooddata].any? { |e| date[:id] == e }).to be_truthy
      expect(date[:urn]&.include?(GoodData::LCM2::SynchronizeDateDimension::DATE_DIMENSION_CUSTOM_V2)).to be_truthy
    end
  ...
  it 'upgrade a date dimension to custom v2' do
    ...
    message = GoodData::LCM2::SynchronizeDateDimension::get_upgrade_message(false, ['datecustom.dataset.dt'])
    to_project&.upgrade_custom_v2(message)
  ...
  it 'upgrade multiple date dimension to custom v2' do
    ...
    message = GoodData::LCM2::SynchronizeDateDimension::get_upgrade_message(false, ['datecustom.dataset.dt','dategooddata.dataset.dt'])
    to_project&.upgrade_custom_v2(message)
